...
RUN git clone --depth 1 -b v2.1.1+backport6012 'https://github.com/thehyve/cbioportal.git' $PORTAL_HOME
WORKDIR $PORTAL_HOME
...
RUN unzip portal/target/cbioportal.war -d $CATALINA_HOME/webapps/cbioportal
RUN mv scripts/target/scripts-*.jar /root/
RUN mvn clean
RUN mkdir scripts/target/
RUN mv /root/scripts-*.jar scripts/target/
